  ...
  def main(args: Array[String]) {
    ...
    if (args.length < 2) {
      ...
      println("DRIVER_MEMORY=[mem] bin/run-example terasort.TeraGen " +
        "[master URI] [output-size] [output-directory]")
      println(" ")
      println("example:")
      println("DRIVER_MEMORY=50g bin/run-example terasort.TeraGen " +
        "spark://141.76.91.165:7077 100G file:///scratch/username/terasort_in")
      System.exit(0)
    }
    ...
    val conf = new SparkConf()
      ...
      //.setExecutorEnv("Memory","1024m")
    //.setMaster(string aus ARGS)
    val sc = new SparkContext(conf)

    val recordsPerPartition = outputSizeInBytes / 100 / noPartitions.toLong
    val numRecords = recordsPerPartition * noPartitions.toLong

    println("===========================================================================")
    println("===========================================================================")
    println(s"Input size: $size")
    println(s"Total number of records: $numRecords")
    println(s"Number of output partitions: $noPartitions")
    println("Number of records/output partition: " + (numRecords / noPartitions))
    println("===========================================================================")
    println("===========================================================================")
    ...
    val dataset = sc.parallelize(1 to noPartitions, noPartitions).mapPartitionsWithIndex { case (index, _) =>
    ...
    println("**********************")
    println("TeraGen/Spark summary")
    println("**********************")
    println("RNG     " + (t1 - t0)/Math.pow(10,9) + "s")
